...
export const aiAPI = {
  ...
  // Candidate-specific interview endpoints
  startCandidateInterview: (jobRole) => 
    api.post('/ai/candidate/interview/start', { jobRole }),
  submitCandidateAnswer: (sessionId, answer, questionId) => 
    api.post('/ai/candidate/interview/answer', { sessionId, answer, questionId }),
...
export const candidateInterviewsAPI = {
  ...
  getMyInterviews: async (params = {}) => {
    // Remove undefined values and add cache-busting timestamp
    const cleanParams = Object.fromEntries(
      Object.entries(params).filter(([_, v]) => v !== undefined)
    );
    cleanParams._t = Date.now(); // Cache-busting timestamp
